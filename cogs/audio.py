...
sys.path.append(os.path.abspath("C:/Users/micb7/PycharmProjects/Prisma/PrismaAUX"))
...
class Audio:
    ...
    async def cache_stats(self):
        """Reports info about the cache.
            ...
            - Minimum cache size. Automatically determined by number of servers Prisma is running on.
        """
    ...
    @commands.command(pass_context=True, no_pm=True)
    async def p(self, ctx, *, url_or_search_terms):
        ...
        if self._match_any_url(url):
            if not self._valid_playable_url(url):
                ...
                url = None
            elif self._valid_playable_url(url):
                url = url_or_search_terms
                add_to_aux(url)
                return
        else:
            result = search_to_url(url)
            url = result
            add_to_aux(url)
        ...
        duration = url_to_sec(url)
        print("New song requested on {} • '{}', {} seconds long.".format(server.name, url_or_search_terms, duration))

        if player.is_playing and self.voice_connected(server):
            await self.bot.say("-")
            ...
            await self.bot.say("-")
            await self._stop_and_disconnect(server)
        elif self.voice_connected(server) and not player.is_playing:
            await asyncio.sleep(duration)
            await self._stop_and_disconnect(server)
        else:
            pass

    @commands.command(pass_context=True, no_pm=True, name="<")
    ...
    @commands.command(pass_context=True, no_pm=True, name="add")
    ...
    @repeat.command(pass_context=True, no_pm=True, name="repeat")
    ...
    async def toggle(self, ctx):
        """Resumes or pauses song/playlist"""
        server = ctx.message.server
        if not self.voice_connected(server):
            await self.bot.say("Not voice connected in this server.")
            return
        ...
        if not hasattr(voice_client, 'audio_player'):
            await self.bot.say("Nothing playing, nothing to pause.")
        elif voice_client.audio_player.is_playing() and not voice_client.audio_player.is_done():
            voice_client.audio_player.pause()
            await self.bot.say("Pausing.")
        else:
            await self.bot.say("Nothing playing, nothing to pause.")

    @commands.command(pass_context=True, aliases=["mix"], no_pm=True)
    async def shuffle(self, ctx):
        """Shuffles the current queue"""
        server = ctx.message.server
        if server.id not in self.queue:
            await self.bot.say("I have nothing in queue to shuffle/mix.")
            return

        self._shuffle_queue(server)
        self._shuffle_temp_queue(server)
        await self.bot.say("Queues shuffled/mixed.")
    ...
    async def sing(self, ctx):
        """Makes Prisma sing one of her songs"""
    ...
    @commands.command(pass_context=True, no_pm=True)
    async def song(self, ctx):  # TODO: AUX's lyrics integration
    ...
    @commands.command(pass_context=True, no_pm=True, name="X")
    ...
    def is_playing(self, server):
        # TODO: Move over the AUX pause/play for On-Demand
        if not self.voice_connected(server):
            return False
...
def search_to_url(queryline):
    query = search(queryline, tld='com', lang='en', num=1, stop=1)
    return query


def add_to_aux(url):
    apath = 'C:/Users/micb7/PycharmProjects/Prisma/PrismaAUX/config/'
    aplist = 'autoplaylist'
    autofull = os.path.join(apath, aplist + ".txt")
    prisma = open(autofull, "a")
    prisma.write(str(url))
    prisma.write("\n")
    prisma.close()
    return
