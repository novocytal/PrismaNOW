...
class Owner:
    ...
    async def _set(self, ctx):
        """Changes prismaNOW's core settings"""
    ...
    async def owner(self, ctx):
        ...
        if self.bot.settings.no_prompt is True:
            await self.bot.say("Console interaction is disabled. Start prismaNOW "
    ...
    async def prefix(self, ctx, *prefixes):
        """Sets prismaNOW's global prefixes
    ...
    async def serverprefix(self, ctx, *prefixes):
        """Sets prismaNOW's prefixes for this server
    ...
    async def game(self, ctx, *, game=None):
        """Sets prismaNOW's playing status
    ...
    async def stream(self, ctx, streamer=None, *, stream_title=None):
        """Sets prismaNOW's streaming status
    ...
    async def _blacklist_add(self, user: discord.Member):
        """Adds user to prismaNOW's global blacklist"""
    ...
    async def _blacklist_remove(self, user: discord.Member):
        """Removes user from prismaNOW's global blacklist"""
    ...
    async def whitelist(self, ctx):
        ...
        be able to use prismaNOW"""
        if ctx.invoked_subcommand is None:
            await self.bot.send_cmd_help(ctx)

    @whitelist.command(name="add")
    async def _whitelist_add(self, user: discord.Member):
        """Adds user to prismaNOW's global whitelist"""
    ...
    async def _whitelist_remove(self, user: discord.Member):
        """Removes user from prismaNOW's global whitelist"""
    ...
    async def restart(self, silently : bool=False):
        """Attempts to restart prismaNOW

        Makes prismaNOW quit with exit code 26
    ...
    @checks.is_owner()
    async def join(self):
        """Shows prismaNOW's invite URL"""
        if self.bot.user.bot:
            await self.bot.whisper("Invite URL: " + "http://prismanow.us.to/")
        else:
    ...
    async def leave(self, ctx):
        ...
        author = ctx.message.author
        message = ctx.message
        await self.bot.say("Are you sure you want me to leave this server? **Say yes, or I will be on your account "
                           "like the NSA to see if you are the real {}. I want out. :sob:**".format(author.name))
        response = await self.bot.wait_for_message(author=message.author)

        if response.content.lower().strip() == 'yes':
            await self.bot.say("Alright. Bye. :wave: :joy:")
            await self.bot.say("http://gph.is/1vbwvYG")
            log.debug('Leaving "{}"'.format(message.server.name))
            await self.bot.leave_server(message.server)
        else:
            await self.bot.say("Okay, I'll stay here then. :weary:")
    ...
    async def info(self):
        ...
        red_repo = "http://prismanow.us.to/"
        author_repo = "https://www.youtube.com/channel/UCVLli-mx9BTFSV87O0EcK9Q"
        sec_repo = "https://github.com/Twentysix26"
        server_url = "https://discord.gg/6HPCfcK"
        dpy_repo = "https://github.com/Rapptz/discord.py"
        python_url = "https://www.python.org/"
        since = datetime.datetime(2017, 4, 8, 0, 0)
        ...
        about = (
            "This is an instance of [prismaNOW, an open source music and social Discord bot]({}) "
            "created by [novocytal]({}), [Twentysix]({}), and improved by many.\n\n"
            "prismaNOW is backed by a passionate community who contributes and "
            "creates content for everyone to enjoy. [Join us today]({}) "
            "and help us improve!\n\n"
            "".format(red_repo, author_repo, sec_repo, server_url))

        embed = discord.Embed(colour=discord.Colour.orange())
        embed.add_field(name="Instance owned by", value=str(owner))
        embed.add_field(name="Python", value=py_version)
        embed.add_field(name="discord.py", value=dpy_version)
        embed.add_field(name="About prismaNOW", value=about, inline=False)
        embed.set_footer(text="Bringing joy since 08 Mar 2017 (over "
    ...
    async def version(self):
        """Shows prismaNOW's current version"""
    ...
    def _get_version(self):
        if not os.path.isdir(".git"):
            msg = "This instance of prismaNOW hasn't been installed with git."
