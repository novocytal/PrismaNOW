...
INTRO = ("______________________________\n"
         "    prismaNOW 2.0 OFFICIAL    \n"
         "¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯\n")
...
def parse_cli_arguments():
    parser = argparse.ArgumentParser(description="prismaNOW's launcher")
    parser.add_argument("--start", "-s",
                        help="Starts Prisma",
                        action="store_true")
    parser.add_argument("--auto-restart",
                        help="Automatically restarts Prisma in case of issues",
                        action="store_true")
    parser.add_argument("--update-red",
                        help="Updates Prisma (git)",
                        action="store_true")
...
def update_red():
    try:
        ...
        return
    if code == 0:
        print("\nPrisma has been updated.")
    else:
        print("\nPrisma could not update properly. If this is caused by edits "
...
def reset_red(reqs=False, data=False, cogs=False, git_reset=False):
    ...
    if git_reset:
        ...
        if code == 0:
            print("Prisma has been restored to the last local commit.")
        else:
...
def update_menu():
    ...
    while True:
        ...
        print("Update:\n")
        print("Prisma:")
        print("1. Update Prisma + requirements (recommended)")
        print("2. Update Prisma")
...
def maintenance_menu():
    ...
    while True:
        print(INTRO)
        print("Maintenance:\n")
        print("1. Repair Prisma (discards code changes, keeps data intact)")
        ...
        if choice == "1":
            ...
            print("Are you sure? This will wipe ALL your Prisma's installation "
...
def run_red(autorestart):
    ...
    if verify_requirements() is None:
        print("You don't have the requirements to start Prisma. "
    ...
    while True:
        try:
            ...
            if code == 0:
                ...
                print("Restarting Prisma...")
                continue
            else:
                if not autorestart:
                    break

    print("Prisma has been terminated. Exit code: %d" % code)
...
def create_fast_start_scripts():
    """Creates scripts for fast boot of prismaNOW without going
...
def main():
    ...
    if IS_WINDOWS:
        os.system("TITLE prismaNOW • Launcher")
    clear_screen()
    ...
    while True:
        ...
        if not is_git_installation:
            print("WARNING: It doesn't look like Prisma has been "
        ...
        print("1. Run Prisma /w autorestart in case of issues")
        print("2. Run Prisma")
        print("3. Update")
...
if __name__ == '__main__':
    ...
    os.chdir(dirname)
    if not PYTHON_OK:
        print("Prisma needs Python 3.5+. Install the required "
              "version.\nPress enter to continue.")
        if INTERACTIVE_MODE:
            wait()
        exit(1)
    if pip is None:
        print("Prisma cannot work without the PIP module. Please make sure to "
